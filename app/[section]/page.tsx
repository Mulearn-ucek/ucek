...
export async function generateMetadata({ params }: { params: { section: string } }) {
  ...
  try{
    ...
    title = (params.section == "departments" && "Departments") || 
...
const getArticle = (({section}:{ section: string }) => {
  ...
  try{
    ...
    title = (section == "departments" && "Departments") || 
    (section == "about" && "About") || 
    ...
    contents = files.map((file) => {
      ...
      return {link: file.replace(".md","") , title: fileContent.split('\n')[0]}
    })
  ...
  return { title, contents} as { title: string, contents: string | {title: string, link: string}[] }
})
  ...
  return (<>
   ...
   <div className='h-[30%] relative text-center -z-20'>
    ...
    <div className='z-20 w-full p-5 flex justify-center'>
      {title != "404" &&  <div className='mb-3 grid grid-flow-dense grid-cols-1 md:grid-cols-2 lg:grid-cols-3 row-auto gap-2'>
              {typeof contents != "string" && contents.map((content) => <Link key={content.link} href={`/${params.section}/${content.link}`} className='flex border p-5 scale-100 rounded hover:scale-105 transition-transform items-center gap-2'><LinkIcon size={20}/> {content.title}</Link>)
                }
          </div>}
    </div>
